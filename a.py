#!/usr/bin/env python3
"""
Integrated Squash Ball Tracking and Analysis System
This file combines ball tracking and game analysis functionality.
"""
import cv2
import numpy as np
import torch
...
def track_squash_ball(
    ...
    Returns:
        tuple: (output_video_path, ball_positions_csv_path, output_dir)
    """
...
def analyze_ball_metrics(ball_positions, fps, height=None):
    ...
    for i in range(3, len(ball_positions)-3):
        ...
        if vy_prev > bounce_threshold and vy_next < -bounce_threshold:
            ...
            if height is None or y_positions[i] > height * 0.4:  # Adjust based on court position
    ...
    court_coverage = {}
    if len(ball_positions) > 0 and isinstance(ball_positions[0], tuple) and len(ball_positions[0]) > 9:
        # Count occurrences of each court region
        court_regions = [pos[9] for pos in ball_positions if pos[9]]
        for region in court_regions:
            if region in court_coverage:
                court_coverage[region] += 1
            else:
                court_coverage[region] = 1
